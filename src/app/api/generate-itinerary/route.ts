...
export async function POST(request: NextRequest) {
  try {
    ...
    // Return mock itinerary for demo (database fallback)
    const mockItinerary = generateMockItinerary(originCity, days, budget, startDate);
    return NextResponse.json(mockItinerary);
    
  } catch (error) {
    console.error('Itinerary generation error:', error);
    // Return mock data on any error
    const mockItinerary = generateMockItinerary('Your City', 3, 30000, new Date().toISOString());
    return NextResponse.json(mockItinerary);
  }
}
...
function generateMockItinerary(city: string, days: number, budget: number, startDate: any) {
  ...
  const dayItineraries = Array.from({ length: Math.min(days, 4) }, (_, dayIdx) => ({
    ...
    places: places.slice(dayIdx * 2, (dayIdx + 1) * 2).map((p, i) => ({
      ...p,
